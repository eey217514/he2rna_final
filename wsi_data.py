...
def load_labels(transcriptome_dataset):
    ...
    to_drop = ['Case.ID', 'Sample.ID', 'File.ID_x', 'File.ID_y', 'Project.ID']
...
def match_patient_split(dataset, split):
    ...
    #print("dataset ...................",(dataset.patients[:, np.newaxis]).shape)
    #print("train..............", (np.array(train_patients).reshape(-1,1)).shape)
    indices = np.arange(len(dataset))
    #matching_indices = np.where(np.isin(dataset.patients[:,np.newaxis][:, 0], np.array(train_patients).reshape(-1,1)[:, 0]))
    #print("idx................", matching_indices)
    train_idx = indices[np.any(dataset.patients[:, np.newaxis] ==
                               np.array(train_patients).reshape(-1,1), axis=0)]
    valid_idx = indices[np.any(dataset.patients[:, np.newaxis] ==
                               np.array(valid_patients).reshape(-1,1), axis=0)]
    test_idx = indices[np.any(dataset.patients[:, np.newaxis] ==
                              np.array(test_patients).reshape(-1,1), axis=0)]

    patient_to_index = {p:i for i, p in enumerate(dataset.patients)}
    train_idx = [i for i in indices if dataset.patients[i] in train_patients[0]]
    valid_idx = [i for i in indices if dataset.patients[i] in valid_patients[0]]
    test_idx = [i for i in indices if dataset.patients[i] in test_patients[0]]
