...
def save_numpy_features(path2slides, folder, slidename, coords, path):
    ...
    slide = openslide.OpenSlide(os.path.join(path2slides, folder, slidename, 'he_aligned.svs'))
...
def process_all_slides(path2slides, tile_coords, path):
    ...
    for folder, slidename in zip(subfolders, slidenames):
        if slidename in tile_coords.keys():
            if os.path.exists(os.path.join(path, '0.50_mpp', slidename.split('.')[0] + '.npy')):
            #if os.path.join(path, '0.50_mpp', slidename.split('.')[0] + '.npy') in os.path.join(path, '0.50_mpp'):
                print("npy file found so skipping")
                continue
