...
class Experiment(object):
    ...
    def _build_dataset(self):
        ...
        if 'genes' in dic.keys():
            ...
            if os.path.exists(genes):
                ...
                genes = genes.split(',')
                for gene in genes:
                    assert gene.startswith('EPCAM'), "Unknown gene format"
        else:
            genes = ['EPCAM', 'COL3A1', 'CD3E', 'PLVAP', 'C1QA', 'IL1B', 'MS4A1', 'CD79A']
    ...
    def single_run(self, random_state=0, logdir='./exp'):
        ...
        dataset.projects = pd.Series(['TCGA-COADREAD']*len(dataset.projects))
