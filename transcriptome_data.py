...
class TranscriptomeDataset:
    ...
    def from_saved_file(cls, path, projectname=None, genes=None):
        ...
        if genes is None:
            ...
            usecols = list(genes) + ['File.ID_x', 'File.ID_y', 'Sample.ID', 'Case.ID', 'Project.ID']
        transcriptomes = pd.read_csv(path + '/' + 'all_transcriptomes.csv', usecols=usecols, engine='python')
        ...
        genes = [col for col in transcriptomes.columns if col in ['EPCAM', 'COL3A1', 'CD3E', 'PLVAP', 'C1QA', 'IL1B', 'MS4A1', 'CD79A']]
    ...
    def _get_infos_on_tiles(self, subdirs, zoom='0.50_mpp'):
        ...
        """
        #print("Followings are the subdir", subdirs)
        #print(PATH_TO_TILES)
        if subdirs is not None:
            df = []
            for subdir in subdirs:
                dir_tiles = os.path.join(PATH_TO_TILES, subdir)
            ...
            for subdir in os.listdir(PATH_TO_TILES):
                ...
                if os.path.isdir(os.path.join(PATH_TO_TILES, subdir)):
    ...
    def _match_data(self):
        ...
        #    lambda x: x[:-1])
        self.transcriptome_metadata.loc[:, 'Sample'] = self.transcriptome_metadata['Sample.ID'].apply(lambda x: x[:-1])
        self.image_metadata.loc[:, 'Sample'] = self.image_metadata['Sample.ID_image'].apply(lambda x: x[:-1])
    ...
    def load_transcriptomes(self):
        ...
        df = pd.read_csv(os.path.join(
            ...
            'transcriptome_fpkmuq_allsamps.csv'), usecols=self.genes, index_col=0)
